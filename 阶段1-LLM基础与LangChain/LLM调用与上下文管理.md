...
- Chat API 的核心输入是模型名称和消息列表。
- `system` 定义角色、规则和安全边界，`user` 表示用户输入，`assistant` 表示历史回复，`tool` 表示工具执行结果。
- 模型不会自动记住历史对话，多轮对话需要由应用保存历史并重新组装消息。
- 应将供应商 SDK 隔离在 `LLMClient` 或 Client 层，业务代码只依赖统一接口，便于切换模型供应商或本地模型。

### 2. Token 和上下文窗口

- Token 是模型处理文本的基本单位，不等同于字符、汉字或单词。
- 上下文预算包括 system prompt、历史消息、当前输入、工具定义、工具结果和预留输出空间。
- 计算优先使用目标模型对应的 tokenizer；没有 tokenizer 时只能进行粗略估算。
- 请求前用 tokenizer 估算是否超限，请求后读取 `usage` 统计真实消耗和成本。
- 上下文预算不能只看输入，还必须为模型输出预留 `max_tokens` 或等价空间。

### 3. 上下文管理

- 优先保留安全规则、当前任务目标和高相关检索内容。
- 对历史消息设置数量或 Token 上限，不能无限累积。
- 超长时可以依次采用：压缩工具结果、删除低相关历史、生成摘要、转移到 RAG。
- 工具返回值只保留业务真正需要的字段，避免把大段日志或无关数据放入上下文。
- 上下文越长不代表效果越好，还要关注成本、延迟和模型注意力下降。

### 4. 采样参数

- `temperature` 调整概率分布的随机性；越低通常越稳定，越高通常越有变化。
- `top-p` 按累计概率截断候选 Token 的范围；越低候选越集中，越高表达越丰富。
- `max_tokens` 只限制模型输出长度，不等于上下文窗口大小。
- 调参时一次只改变一个参数，并使用固定测试集比较准确率、格式合规率、重复率和成本。
- 结构化抽取、分类和 Tool Calling 通常使用较低随机性；创意生成可以适当提高随机性。

### 5. Prompt 设计

一个可执行的 system prompt 至少应明确：

```text
角色 -> 目标 -> 输入格式 -> 输出格式 -> 工具规则 -> 安全边界 -> 不确定时的处理方式
```

- 安全边界不能只依赖用户消息，服务端仍必须执行权限校验。
- Prompt 负责指导模型，权限、数据隔离和危险操作确认必须由服务端代码保证。

### 6. Pydantic 和结构化结果

- 用 Pydantic 模型描述请求和模型输出，而不是依赖字符串截取或正则表达式。
- `Field` 可以限制长度、范围和格式，`Literal` 可以限制枚举值。
- 使用 `model_validate()` 校验字典结果，校验失败时进入有限重试、修复或人工处理流程。
- 结构化输出不仅要格式正确，还要满足业务规则，例如订单号必须为正数、置信度必须在 `0` 到 `1` 之间。

### 7. 练习验收清单

完成下面任务，基本说明本文重点已经掌握：

1. 能组装包含 system、历史消息和当前输入的消息列表。
2. 能限制历史消息数量，并根据 Token 预算截断历史。
3. 能解释 `temperature`、`top-p` 和 `max_tokens` 的区别。
4. 能使用 tokenizer 估算输入 Token，并读取 API 返回的 `usage`。
5. 能定义 `LLMClient` Protocol，并用 Fake Client 编写测试。
6. 能使用 Pydantic 校验模型结构化输出。
7. 能处理模型返回缺字段、错误枚举、非法 JSON 和业务规则不满足的情况。

本文的核心目标不是记住某一家模型 SDK 的调用方法，而是建立一条稳定的 LLM 应用链路：
